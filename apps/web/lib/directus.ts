...
export const fallbackDevicePageSettings: DevicePageSettings = {
  ...
  sections: {
    ...
    warrantyTitle: "Гарантия и условия обновления",
    warrantyDurationLabel: "Срок гарантии",
    exitPriceLabel: "Предварительная стоимость при обновлении",
    ...
    tradeValuePrefix: "Предварительная оценка",
    tradeCtaLabel: "Получить предварительную оценку",
    tradeCtaHref: "/trade",
...
export function mapDeviceFromDirectus(
  ...
  return {
    ...
    tags: json<string[]>(row.tags, []),
    category: str(row.category),
    brand: str(row.brand, "Apple"),
    year: num(row.year) || undefined,
    modelIdentifier: str(row.model_identifier),
    region: str(row.region),
    sim: str(row.sim),
    batteryCycles: num(row.battery_cycles) || undefined,
    diagnosticDate: str(row.diagnostic_date),
    activationLock: str(row.activation_lock),
    mdm: str(row.mdm),
    completeness: str(row.completeness),
    diagnosticBy: str(row.diagnostic_by),
    sort: num(row.sort),
    title: str(row.title),
    model: str(row.model),
    specs: str(row.specs),
...
function mapSiteSettingsFromDirectus(row: Record<string, unknown>): SiteSettings {
  ...
  return {
    ...
    footerContactEyebrow: str(row.footer_contact_eyebrow),
    footerMapLabel: str(row.footer_map_label),
    footerStoreLabel: str(row.footer_store_label),
    footerContactHeading: str(row.footer_contact_heading),
    footerHoursHeading: str(row.footer_hours_heading),
    footerSellerLabel: str(row.footer_seller_label),
    footerLegalAddressLabel: str(row.footer_legal_address_label),
    footerContactsFallback: str(row.footer_contacts_fallback),
    footerHoursFallback: str(row.footer_hours_fallback),
    footerNetworkEyebrow: str(row.footer_network_eyebrow),
    footerNetworkTitle: str(row.footer_network_title),
    footerNetworkBody: str(row.footer_network_body),
    footerAllStoresLabel: str(row.footer_all_stores_label),
