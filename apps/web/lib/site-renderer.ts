...
function marketingItems(value: unknown): { title: string; text: string; badge: string; url: string; label: string }[] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((item, index) => {
    if (!item || typeof item !== "object") return [];
    const record = item as Record<string, unknown>;
...
function renderPageHeroSection(section: PageSection): string {
  return `<section class="page-hero wrap">
  ${section.eyebrow ? `<div class="breadcrumb-pill">${escapeHtml(section.eyebrow)}</div>` : ""}
  ${section.headline ? `<h1 class="display">${escapeHtml(section.headline)}</h1>` : ""}
  ${section.body ? `<p class="lead">${escapeHtml(section.body)}</p>` : ""}
  ${
    section.primaryCtaLabel || section.secondaryCtaLabel
      ? `<div class="btn-row" style="margin-top:32px;">
      ${
        section.primaryCtaLabel
          ? `<a class="btn btn--filled" href="${escapeHtml(normalizeSiteUrl(section.primaryCtaUrl || "/#final"))}">${escapeHtml(section.primaryCtaLabel)}</a>`
          : ""
      }
      ${
        section.secondaryCtaLabel
          ? `<a class="btn btn--outlined" href="${escapeHtml(normalizeSiteUrl(section.secondaryCtaUrl || "/catalog"))}">${escapeHtml(section.secondaryCtaLabel)}</a>`
          : ""
      }
    </div>`
      : ""
...
function renderVisualBandSection(section: PageSection): string {
  const visual = visualContent(section.content.visual);
  const image = section.image || visual.imageSrc;
  return `<section class="section" style="padding-top:0;">
  <div class="wrap">
    <div class="store-visual" style="margin-top:0;">
      ${image ? `<img src="${escapeHtml(image)}" alt="${escapeHtml(visual.imageAlt || section.headline || "")}" />` : ""}
      <div class="store-visual__caption">
        <strong>${escapeHtml(visual.captionTitle || section.headline || "")}</strong>
        <span>${escapeHtml(visual.captionText || section.body || "")}</span>
      </div>
    </div>
  </div>
</section>
...
function renderCardsGridSection(section: PageSection): string {
  const cards = marketingItems(section.content.items ?? section.content.cards);
  return `<section class="section${section.variant?.includes("wash") ? " section--wash" : ""}">
  <div class="wrap">
    <div class="sec-head">
      ${section.eyebrow ? `<div class="eyebrow">${escapeHtml(section.eyebrow)}</div>` : ""}
      ${section.headline ? `<h2 class="h2">${escapeHtml(section.headline)}</h2>` : ""}
...
function renderStepsSection(section: PageSection): string {
  const steps = sectionItemList(section.content.steps);
  ...
  <div class="wrap">
    ...
    <div class="steps">
      ${steps
        ...
        <div class="step__num">${String(index + 1).padStart(2, "0")}</div>
        <div class="step__title">${escapeHtml(step.title)}</div>
        <div class="step__desc">${escapeHtml(step.text)}</div>
      </div>`,
        )
        .join("\n      ")}
    </div>
...
function renderMarketingCompareSection(section: PageSection): string {
  const comparison = comparisonContent(section.content.comparison);
  ...
  <div class="wrap">
    ...
    <div class="compare" role="table" aria-label="${escapeHtml(comparison.ariaLabel)}">
      <div class="compare__row compare__head" role="row">
        <div class="compare__cell compare__cell--label" role="columnheader">${escapeHtml(comparison.labelHeader)}</div>
        <div class="compare__cell compare__cell--bad" role="columnheader">${escapeHtml(comparison.badHeader)}</div>
        <div class="compare__cell compare__cell--good" role="columnheader">${escapeHtml(comparison.goodHeader)}</div>
...
function renderMarketingLevelsSection(section: PageSection): string {
  const levels = clubLevelList(section.content.levels);
  return `<section class="section section--carbon">
  <div class="wrap">
    <div class="sec-head">
      ${section.eyebrow ? `<div class="eyebrow">${escapeHtml(section.eyebrow)}</div>` : ""}
      ${section.headline ? `<h2 class="h2">${escapeHtml(section.headline)}</h2>` : ""}
    ...
    <div class="club-levels">
      ${levels
        .map(
          (level) => `<div class="level${level.featured ? " level--featured" : ""}">
        ${level.badge ? `<div class="level__badge">${escapeHtml(level.badge)}</div>` : ""}
        <div class="level__name">${escapeHtml(level.name)}</div>
        <div class="level__tag">${escapeHtml(level.tag)}</div>
        <ul class="level__features">
...
function renderFaqSection(section: PageSection): string {
  const items = marketingItems(section.content.items);
  return `<section class="section">
  <div class="wrap">
    <div class="sec-head">
      ${section.eyebrow ? `<div class="eyebrow">${escapeHtml(section.eyebrow)}</div>` : ""}
    ...
    <div class="passport-accordion" style="max-width:880px;margin:48px auto 0;">
      ${items
        .map(
          (item) => `<details class="passport-detail">
        <summary><span>${escapeHtml(item.title)}</span><strong>${escapeHtml(item.badge)}</strong></summary>
        <div class="passport-detail__body"><p style="padding-top:24px;color:var(--color-graphite);">${escapeHtml(item.text)}</p></div>
      </details>`,
        )
        .join("\n      ")}
    </div>
...
function renderPageCtaSection(section: PageSection): string {
  return `<section class="section">
  <div class="wrap">
...
function renderMarketingSection(section: PageSection): string {
  if (section.variant === "page.hero" || section.sectionKey.endsWith("_hero")) return renderPageHeroSection(section);
  if (section.variant === "visual.band") return renderVisualBandSection(section);
  if (section.variant === "steps" || section.sectionKey.endsWith("_steps")) return renderStepsSection(section);
  if (section.variant === "compare" || section.sectionKey.endsWith("_compare")) return renderMarketingCompareSection(section);
  if (section.variant === "levels" || section.sectionKey === "club_levels") return renderMarketingLevelsSection(section);
  if (section.variant === "faq" || section.sectionKey === "faq") return renderFaqSection(section);
  if (section.variant === "page.cta" || section.sectionKey === "final_cta") return renderPageCtaSection(section);
  if (section.variant === "cards.grid" || section.content.items || section.content.cards) return renderCardsGridSection(section);
  return "";
}
