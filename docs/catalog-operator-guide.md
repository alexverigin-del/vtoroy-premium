...
После импорта проверьте результат в `Каталог` -> `Товары` (`products`) и в
связанной группе `Фото` (`product_images`). Старые `devices` и `device_images`
не являются рабочим интерфейсом редактора.
...
Demo batch создаёт служебный товар в `draft` / `hidden`; он не появляется на
сайте и нужен только для проверки маршрута workbook → ZIP → Files → import.

Важно: новые товарные фото должны появляться только через `product_images` и
`products.listing_file`. Не вставляйте фото вручную в JSON-поля,
`/assets/...`, `https://api.isvoi.ru/assets/...` или внешние URL. Legacy-поля
сохраняются только на время dual-read и не редактируются оператором.
...
- `products`: цена, описание, тип товара, статус наличия и `content_status`;
- связанное `product_images`: есть главное и дополнительные фото с alt-текстом;
- `directus_files`: фото лежат в `ISVOI Device Photos`.
- В JSON-полях нет вручную вставленных ссылок на товарные фото.
