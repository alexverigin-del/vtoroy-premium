...
В Studio она находится в разделе `Карточки сайта` → `Товары`. Да, раздел
`Карточки сайта` — это редакторский контур Catalog V3. Публичный каталог
...
6. Для б/у техники добавьте Passport, дату и исполнителя диагностики, грейд,
...
- Б/у техника без Passport, даты и исполнителя диагностики, грейда, хотя бы
...
В карточке б/у техники откройте группу `Passport`. Поля `Краткие факты` и
