...
не является публичной страницей магазина. Путь `/store` постоянно
перенаправляется на `/belgorod`. Не публикуйте `store`, пока для неё не будет
утверждён отдельный сценарий, отличающийся от городского хаба.
