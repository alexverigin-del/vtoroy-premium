...
Открывайте `Склад и каналы` → `Магазины и города` → нужный город. Для каждого
города отдельно заполняются:
...
Перед публикацией нового города проверьте футер и страницу контактов именно с
этим выбранным городом. Запись без адреса, телефона, часов, карты, продавца,
