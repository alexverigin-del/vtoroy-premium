...
- The user explicitly authorized the release. The feature release is `e49484b`; production and GitHub `master` subsequently advanced to network-resilience hotfix `a45255a`. Backup `/opt/isvoi/backups/telegram-notifications-20260905T164908332Z` was created and verified by the release gate.
- Full server `web:verify` and the isolated PostgreSQL/Directus rehearsal passed. The rehearsal covers production policies, support intake, explicit consent, immutable logs and snapshots, deduplication, service priority, CTA validation, quiet hours, frequency suppression, 429, blocked/uncertain delivery, opt-out cancellation and six-month retention.
- Production has all five notification collections, three active topics, `telegram_message_outbox.state varchar(32)`, notification and pilot switches enabled, allowlist `65092546`, and channel delivery disabled. Existing users remain unsubscribed.
...
- Production release `e4e0274` completed through the pinned gate after local 22/22 tests, full server `web:verify` and an isolated PostgreSQL/Directus production-policy contract. Verified backup: `/opt/isvoi/backups/telegram-speed-20260905T182855552Z`. Final checks show a clean checkout, online worker with no unstable restarts, live lease, empty active queue, five expected delivery columns, two initialized metric rows and Directus `pong`.
- Post-release observation found that a transient Telegram poll failure let PM2 restart with a new worker identity while the prior 90-second lease was still live, producing temporary `DIRECTUS_HTTP_409` restarts. Production hotfix `de59e84` now retries transient poll/network/lease errors in-process with the same identity; an unacknowledged outbound result also holds new delivery until its in-flight deadline instead of repeating the Telegram call. Server tests pass 23/23, and the worker remained online without another restart beyond the full lease window; lease is live and the active queue is empty.
- Telegram release helpers use `docker compose up -d --force-recreate --no-deps directus`. A plain `up -d` can leave an unchanged running container intact and therefore keep the previous mounted extension code; force recreation both reloads extensions and applies any env changes before the health gate.
- Pilot subscription `news_promotions` for Telegram `65092546` recorded explicit consent `pilot-2026-09`. Test campaign `f05d36f2-31df-4ad5-9aa6-f12d21cdf501` delivered a verified Directus WebP and first-party CTA as Telegram message `28`; `sent_at=2026-09-05T19:04:33.419159Z`, queue-ready latency `6678 ms`. The ordinary after-hours recipient row was cancelled as `TEST_ONLY_NO_DUPLICATE`, so no duplicate is scheduled for the next marketing window.

### Telegram Branded Welcome · 2026-09-05

- Production `master` is `8696c93`. `/start` now sends a single `sendPhoto` welcome containing the brand image, greeting and five-action keyboard. Opening the general deep link still creates no lead; a missing, SVG or invalid configured file falls back to the existing text welcome.
- `telegram_bot_settings.welcome_photo_file` is a managed Directus image relation. Production uses file `0c339e2e-06b0-45f7-a0d5-b22086a119f2` (`image/jpeg`, 1672×941, 105009 bytes). The file is public through the checked Directus asset endpoint and the active queue is empty.
