...
- Blog Phase 1–3 was committed, pushed and deployed to production on
  ...
  followed at `18c7860`. Production navigation intentionally remains unchanged.
...
- Editorial states are `draft`, `review`, `scheduled`, `published` and
  ...
  published rows.
- `blog_posts` uses Directus Content Versioning. Live Preview is prepared by
...
- The nginx example disables access logging for `/api/draft/blog` because the
  ...
  or committed. Production Live Preview is not enabled yet: the `deploy` user
  can read the active vhost but has no passwordless sudo to edit/test/reload
  nginx. `BLOG_PREVIEW_SECRET`, `DIRECTUS_PREVIEW_TOKEN` and the Directus
  `preview_url` remain unset, so `/api/draft/blog` fails closed with 401 and no
  preview secret is written to the normal nginx access log.
...
- The idempotent `directus:seed:blog-pilot` command created category
  ...
  `chto-pokazyvaet-diagnostika-iphone`, linked to `iphone-13-pro`. The pilot is
  `draft`, `no_index=true`, has zero published rows and returns 404 publicly.
  Its current diagnostics image is a temporary existing site asset; replace it
  with an approved file in `ISVOI Blog` during Studio QA.
- Schema, catalog, image, navigation, legacy, leads, files, import, Studio and
  page-section production audits pass. The only intentional blog audit failure
  is `blog.studio.preview_url_missing=1` until root-level nginx configuration is
  available. Do not add the Blog navigation item or publish the pilot before
  Live Preview, version history, scheduling and revalidation QA are completed.
