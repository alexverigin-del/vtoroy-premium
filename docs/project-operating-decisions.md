...
- Post-release observation found that a transient Telegram poll failure let PM2 restart with a new worker identity while the prior 90-second lease was still live, producing temporary `DIRECTUS_HTTP_409` restarts. The worker now retries transient poll/network/lease errors in-process with the same identity; an unacknowledged outbound result also holds new delivery until its in-flight deadline instead of repeating the Telegram call.
