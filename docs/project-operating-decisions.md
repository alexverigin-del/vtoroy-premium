...
- Current frontend styling is intentionally hybrid:
  - `apps/web/app/globals.css` loads Tailwind base/components/utilities and
    ...
    and `.card`.
  - `apps/web/app/site.css` owns the large renderer/legacy marketing layer
    generated by `apps/web/lib/site-renderer.ts`.
...
- Keep `site.css` as a transition layer for renderer-generated HTML while
  marketing sections are still emitted as string markup.
