...
- Повторяемая вводная часть секций собрана в `HomeSectionIntro`: desktop
  использует 12-колоночную split-композицию, mobile — естественный одноколоночный
  поток. Центрированная подача остаётся исключением для Hero, а не шаблоном всех
  блоков.
- Eyebrow занимает отдельную строку над рабочей сеткой секции. На desktop начало
  H2 слева и вводного текста справа должно находиться на одной горизонтали;
  выравнивание колонок по нижнему краю запрещено. Каноническая секционная
  типографика: H2 `48px/600` на desktop и `30px/600` на mobile, вводный текст
  `16px/400`. Passport, FAQ и Final CTA следуют той же верхней горизонтали, Hero
  остаётся отдельной центрированной композицией.
