...
- `npm audit --omit=dev` после релиза показывает 4 high и 1 moderate в production dependency chain. Исправлять отдельным dependency-hardening релизом с полными regression, visual и performance gates; автоматический `--force` не применять.
