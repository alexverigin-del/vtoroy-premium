...
- Перед применением создан backup `/opt/isvoi/backups/directus/20260830T175012Z`. Аудиты `trade-governance`, `trade-legal` и `trade-mvp` прошли без блокеров.
- Контроль актуальных карточек нашёл 17 доступных целей обмена. Нет опубликованных складских предложений, ошибочно исключённых сводным остатком товара, и нет карточек без листингового изображения.
- Новые карточки подхватываются автоматически по фактическому состоянию `products`, `product_offers` и остатков. Юридическая миграция не меняет публикацию, фото, Passport или остатки карточек.
- `trade_settings` и feature flag не публиковались: калькулятор остаётся выключенным до отдельного решения.
- Production web обновлён до `3baae5a`. Связанные release-коммиты: `bcce905` — reviewed fallback copy, `077d9aa` — изоляция Trade‑in клиентского бандла, `b417c73` — точный runtime permission contract.
- Финальный permission apply выполнен после rehearsal с `ROLLBACK` и backup `/opt/isvoi/backups/directus/20260830T182244Z`. Штатная команда удалила только 19 ключей `permissions:*`, перезапустила Directus и получила health `ok`.
- `trade-runtime`, `trade-governance` и `trade-legal` прошли без блокеров. Сервис читает полный юридический singleton и отсортированные магазины; Public по-прежнему не имеет прямого доступа к `trade_*`.
- Закрытый QA v2 прошёл: 19 конфигураций, 7 вопросов, 10 контрольных расчётов, неверный телефон отклоняется, отсутствие согласия отклоняется до idempotency replay, повтор валидной заявки возвращает прежний номер.
- Тестовая заявка `QA-260830-861` сохранена только как QA. Read-only SQL-проверка подтвердила версию `trade-consent-v1-2026-08-30`, время, полный снимок текста, SHA-256 и источник `/trade/qa`; контакт в документации не фиксируется.
- Production `web:verify`, bundle budget, основной HTTP smoke и copy smoke зелёные. Club route после изоляции Trade‑in кода — 418,3 кБ raw при лимите 460 кБ.
- Dependency hardening закрыт коммитом `ce5290a`: Next.js `15.5.21`, Sanitize HTML `2.17.7`, PostCSS `8.5.26`, Nanoid `3.3.18`, Sharp `0.35.4`; полный и production-only `npm audit` возвращают `0 vulnerabilities`, `--force` не применялся.
- Regression-коммит `4a7372d` сохраняет валидный Trade CTA `#final`, пока калькулятор выключен, и переводит legacy Club CTA на `#club-request`. `7e1fd8b` проверяет до четырёх фактически опубликованных карточек вместо удалённых demo-slug.
- После выкатки прошли `web:verify`, bundle budget, route/copy/link/consistency, desktop/mobile visual, performance, закрытый Trade QA и три Directus-аудита. Проверка обмена по-прежнему видит 17 карточек; `TRADE_WIZARD_ENABLED=0` не менялся.
