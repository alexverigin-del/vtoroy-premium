...
import { TRADE_PRICING_VERSION_V2, tradePricingConfigsV2 } from "./trade_pricing_v2_data.mjs";

const sqlText = (value) => `'${String(value).replaceAll("'", "''")}'`;
const expectedValues = tradePricingConfigsV2
  .map(
    (item) =>
...
WITH expected(model_slug,storage,base_min,base_max,sort) AS (VALUES
  ...
  FROM expected
  FULL JOIN actual USING(model_slug,storage)
  WHERE expected.model_slug IS NULL OR actual.model_slug IS NULL
    OR actual.status<>'draft'
    OR expected.base_min<>actual.base_min OR expected.base_max<>actual.base_max OR expected.sort<>actual.sort
)
SELECT 'trade_pricing_v2.version_missing' AS check_name,(1-count(*))::text AS value
FROM trade_pricing_versions WHERE version=${sqlText(TRADE_PRICING_VERSION_V2)} AND status='draft'
UNION ALL
SELECT 'trade_pricing_v2.config_mismatches',count(*)::text FROM config_mismatches
UNION ALL
SELECT 'trade_pricing_v2.rules_invalid',
  CASE WHEN count(*)=21 AND count(*) FILTER(WHERE rule.status='draft')=21 THEN '0' ELSE '1' END
FROM trade_condition_rules rule
...
SELECT 'trade_pricing_v2.previous_draft_invalid',
  CASE WHEN
    (SELECT count(*) FROM trade_pricing_versions WHERE version='trade-mvp-2026-08-29-draft' AND status='draft')=1
    AND (SELECT count(*) FROM trade_device_configs config JOIN trade_pricing_versions version ON version.id=config.pricing_version WHERE version.version='trade-mvp-2026-08-29-draft' AND config.status='draft')=19
    AND (SELECT count(*) FROM trade_condition_rules rule JOIN trade_pricing_versions version ON version.id=rule.pricing_version WHERE version.version='trade-mvp-2026-08-29-draft' AND rule.status='draft')=21
  THEN '0' ELSE '1' END;
`);
