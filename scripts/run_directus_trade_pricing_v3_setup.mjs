...
if (apply && !confirmed) throw new Error("Production apply requires --apply --confirm-production");
...
const governedFiles = [
  ...
  "scripts/seed_directus_trade_pricing_v3_sql.mjs",
  "scripts/audit_directus_trade_pricing_v3_sql.mjs",
  "scripts/run_directus_trade_pricing_v3_setup.mjs",
];
if (apply) {
...
const sql = run(process.execPath, [
  path.join(root, "scripts", "seed_directus_trade_pricing_v3_sql.mjs"),
  ...(apply ? [] : ["--rehearse"]),
]);
...
console.log(`Trade-in pricing v3 setup: ${apply ? "APPLIED" : "REHEARSED"}`);
