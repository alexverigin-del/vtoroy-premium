...
SELECT isvoi_blog_upsert_field('blog_posts','body','input-rich-text-html',NULL,json_build_object('toolbar',json_build_array('bold','italic','bullist','numlist','blockquote','link','removeformat'),'folder',(SELECT id FROM directus_folders WHERE name='ISVOI Blog' AND parent IS NULL LIMIT 1)),'full',126,'Устаревший текст сохранён для обратной совместимости. Новые материалы собирайте в поле «Блоки статьи».',true,true,false,NULL,'group_system','Устаревший текст');

SELECT isvoi_blog_upsert_field('blog_posts','cover_image','file-image','image',json_build_object('folder',(SELECT id FROM directus_folders WHERE name='ISVOI Blog' AND parent IS NULL LIMIT 1)),'half',51,'Основная обложка из папки ISVOI Blog.',false,false,false,'m2o','group_media','Обложка');
...
SELECT isvoi_blog_upsert_field('blog_posts','og_image','file-image','image',json_build_object('folder',(SELECT id FROM directus_folders WHERE name='ISVOI Blog' AND parent IS NULL LIMIT 1)),'half',95,'Отдельная social preview-картинка. Если пусто, используется обложка.',false,false,false,'m2o','group_seo','OG image');
...
SELECT isvoi_blog_upsert_field('blog_authors','avatar','file-image','image',json_build_object('folder',(SELECT id FROM directus_folders WHERE name='ISVOI Blog' AND parent IS NULL LIMIT 1)),'half',5,'Портрет из папки ISVOI Blog.',false,false,false,'m2o',NULL,'Фото');
...
SELECT isvoi_blog_upsert_field('blog_post_blocks','body','input-rich-text-html',NULL,json_build_object('toolbar',json_build_array('bold','italic','bullist','numlist','blockquote','link','removeformat'),'folder',(SELECT id FROM directus_folders WHERE name='ISVOI Blog' AND parent IS NULL LIMIT 1)),'full',5,'Текст блока: короткие абзацы, H2/H3, списки, цитаты и ссылки. Для блока изображения оставьте пустым.',false,false,false,NULL,NULL,'Текст');
SELECT isvoi_blog_upsert_field('blog_post_blocks','image','file-image','image',json_build_object('folder',(SELECT id FROM directus_folders WHERE name='ISVOI Blog' AND parent IS NULL LIMIT 1)),'full',6,'Изображение блока. Перед публикацией переместите одобренный файл в ISVOI Editorial.',false,false,false,'m2o',NULL,'Изображение');
...
SELECT isvoi_blog_upsert_permission('ISVOI Editor','blog_posts','read','group_publication,group_content,group_media,group_relations,group_seo,group_system,id,status,slug,title,excerpt,body,cover_image,cover_alt,cover_caption,category,author,featured,publish_at,published_at,seo_title,meta_description,canonical_url,no_index,og_image,date_created,date_updated,user_created,user_updated,tags,devices,blocks',NULL);
SELECT isvoi_blog_upsert_permission('ISVOI Editor','blog_posts','create','group_publication,group_content,group_media,group_relations,group_seo,group_system,status,slug,title,excerpt,body,cover_image,cover_alt,cover_caption,category,author,featured,publish_at,published_at,seo_title,meta_description,canonical_url,no_index,og_image,tags,devices,blocks',NULL,'{"status":{"_in":["draft","review","scheduled","published","archived"]},"slug":{"_regex":"^[a-z0-9]+(?:-[a-z0-9]+)*$"},"title":{"_nnull":true}}'::json,'{"status":"draft","featured":false,"no_index":false}'::json);
SELECT isvoi_blog_upsert_permission('ISVOI Editor','blog_posts','update','group_publication,group_content,group_media,group_relations,group_seo,group_system,status,slug,title,excerpt,body,cover_image,cover_alt,cover_caption,category,author,featured,publish_at,published_at,seo_title,meta_description,canonical_url,no_index,og_image,tags,devices,blocks',NULL,'{"status":{"_in":["draft","review","scheduled","published","archived"]},"slug":{"_regex":"^[a-z0-9]+(?:-[a-z0-9]+)*$"},"title":{"_nnull":true}}'::json);
...
SELECT isvoi_blog_upsert_permission('ISVOI Blog Version Workflow','directus_revisions','read','*','{"collection":{"_eq":"blog_posts"}}'::json);
...
SELECT isvoi_blog_upsert_preset('ISVOI Editor','blog_posts','Редплан','calendar_month','#0071e3','{"status":{"_in":["draft","review","scheduled"]}}'::json,'{"tabular":{"sort":["publish_at","-date_updated"],"fields":["status","publish_at","title","category","author","date_updated"],"page":1}}'::json);
