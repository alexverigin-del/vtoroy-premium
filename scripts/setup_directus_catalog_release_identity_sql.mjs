...
DO $$
DECLARE v_role uuid; v_user uuid; v_manager_policy uuid; v_import_policy uuid;
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE "user"=v_user AND policy=v_manager_policy AND role IS NULL
  ) THEN
    INSERT INTO directus_access(id,role,"user",policy,sort)
    VALUES(gen_random_uuid(),NULL,v_user,v_manager_policy,1);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM directus_access
...
SELECT 'catalog_release.access_policies_ready' AS check_name,count(*)::text AS value
...
WHERE users.email='catalog-release-qa@service.isvoi'
  AND access.role IS NULL AND policy.name IN ('ISVOI Inventory Manager','ISVOI Catalog Import');
`);
