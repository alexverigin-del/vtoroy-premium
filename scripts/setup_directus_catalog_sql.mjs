 ...
 *
 * Usage:
 *   node scripts/setup_directus_catalog_sql.mjs > /tmp/isvoi_setup_directus_catalog_sql.sql
 *   cd infra/directus-beget
 *   set -a && . ./.env && set +a
 *   docker compose exec -T database psql -U "$DB_USER" -d "$DB_DATABASE" -v ON_ERROR_STOP=1 < /tmp/isvoi_setup_directus_catalog_sql.sql
 */
...
SELECT isvoi_upsert_directus_field('devices', 'brand', 'input', NULL, NULL, 'half', 17, 'Бренд устройства. Для Apple-каталога используйте Apple.');
SELECT isvoi_upsert_directus_field('devices', 'year', 'input', NULL, '{"min":2000,"max":2100}'::json, 'half', 18, 'Год выпуска или поколение.');
SELECT isvoi_upsert_directus_field('devices', 'model_identifier', 'input', NULL, NULL, 'half', 19, 'Точный идентификатор модели, если подтверждён диагностикой.');
SELECT isvoi_upsert_directus_field('devices', 'region', 'input', NULL, NULL, 'half', 20, 'Регион устройства.');
SELECT isvoi_upsert_directus_field('devices', 'sim', 'input', NULL, NULL, 'half', 21, 'Конфигурация SIM/eSIM.');
SELECT isvoi_upsert_directus_field('devices', 'battery_cycles', 'input', NULL, '{"min":0}'::json, 'half', 22, 'Количество циклов батареи на дату диагностики.');
SELECT isvoi_upsert_directus_field('devices', 'diagnostic_date', 'datetime', 'datetime', NULL, 'half', 23, 'Дата последней полной диагностики.');
SELECT isvoi_upsert_directus_field('devices', 'activation_lock', 'input', NULL, NULL, 'half', 24, 'Результат проверки Activation Lock.');
SELECT isvoi_upsert_directus_field('devices', 'mdm', 'input', NULL, NULL, 'half', 25, 'Результат проверки MDM.');
SELECT isvoi_upsert_directus_field('devices', 'completeness', 'input-multiline', NULL, NULL, 'full', 26, 'Комплект поставки.');
SELECT isvoi_upsert_directus_field('devices', 'diagnostic_by', 'input', NULL, NULL, 'half', 27, 'Имя или роль ответственного за диагностику.');
