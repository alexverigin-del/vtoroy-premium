...
SELECT isvoi_upsert_directus_field('device_passports', 'summary_rows', 'list', NULL, '{"template":"{{label}}: {{value}}","addLabel":"Добавить факт","fields":[{"field":"label","name":"Показатель","type":"string","meta":{"interface":"input","width":"half","required":true}},{"field":"value","name":"Значение","type":"string","meta":{"interface":"input","width":"half","required":true}},{"field":"state","name":"Оценка","type":"string","meta":{"interface":"select-dropdown","width":"half","required":true,"options":{"choices":[{"text":"Норма","value":"ok"},{"text":"Обратить внимание","value":"warn"},{"text":"Критично","value":"bad"}]}}}]}'::json, 'full', 21, 'Краткие проверяемые факты Passport: показатель, значение и оценка.', false, 'cast-json', 'group_summary');
...
SELECT isvoi_upsert_directus_field('device_passports', 'diagnostics_checklist', 'list', NULL, '{"template":"{{text}} · {{state}}","addLabel":"Добавить проверку","fields":[{"field":"text","name":"Что проверено","type":"string","meta":{"interface":"input","width":"full","required":true}},{"field":"state","name":"Результат","type":"string","meta":{"interface":"select-dropdown","width":"half","required":true,"options":{"choices":[{"text":"Норма","value":"ok"},{"text":"Обратить внимание","value":"warn"},{"text":"Критично","value":"bad"}]}}}]}'::json, 'full', 25, 'Фактический чек-лист диагностики: проверка и результат.', false, 'cast-json', 'group_summary');
...
SELECT isvoi_upsert_directus_field('device_passports', 'condition_notes', 'tags', NULL, '{"placeholder":"Введите факт и нажмите Enter"}'::json, 'full', 53, 'Короткие проверяемые факты о состоянии. Один факт — одна строка; добавляйте пункт клавишей Enter.', false, 'cast-json', 'group_condition');
...
SELECT isvoi_upsert_directus_field('device_passports', 'story_facts', 'tags', NULL, '{"placeholder":"Введите факт и нажмите Enter"}'::json, 'full', 73, 'Короткие подтверждённые факты истории без персональных данных. Один факт — одна строка; добавляйте пункт клавишей Enter.', false, 'cast-json', 'group_story');
