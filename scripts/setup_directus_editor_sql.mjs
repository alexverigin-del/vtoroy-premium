 ...
 *
 * Usage:
 *   node scripts/setup_directus_editor_sql.mjs > /tmp/isvoi_setup_directus_editor_sql.sql
 *   cd infra/directus-beget
 *   set -a && . ./.env && set +a
 *   docker compose exec -T database psql -U "$DB_USER" -d "$DB_DATABASE" -v ON_ERROR_STOP=1 < /tmp/isvoi_setup_directus_editor_sql.sql
 */
...
-- Devices: publishing.
SELECT isvoi_upsert_directus_field('devices', 'id', 'input', NULL, NULL, 'half', 2, 'Публичный slug и primary key. Используется в URL /device/{id}.', false, NULL, 'group_publish', true, NULL, 'Используйте латиницу, цифры и дефисы: iphone-15-pro-256-blue.');
