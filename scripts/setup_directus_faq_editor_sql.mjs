 ...
 *
 * Usage:
 *   node scripts/setup_directus_faq_editor_sql.mjs > /tmp/isvoi_setup_directus_faq_editor_sql.sql
 *   cd infra/directus-beget
 *   set -a && . ./.env && set +a
 *   docker compose exec -T database psql -U "$DB_USER" -d "$DB_DATABASE" -v ON_ERROR_STOP=1 < /tmp/isvoi_setup_directus_faq_editor_sql.sql
 */
...
SELECT isvoi_upsert_directus_field('faq_items', 'question', 'input', NULL, '{"placeholder":"Например: Можно ли посмотреть диагностику?"}'::json, NULL, 'full', 2, 'Короткий вопрос одним предложением. Лучше писать так, как его задал бы клиент.', false, false, true, NULL, 'group_answer', 'Вопрос', '{"question":{"_nnull":true}}'::json, 'Заполните вопрос.');
...
SELECT isvoi_upsert_directus_field('faq_items', 'key', 'input', NULL, '{"placeholder":"passport-warranty"}'::json, NULL, 'half', 91, 'Стабильный ключ латиницей. Используется в page_sections.content.faqKeys/faq_keys для ручного выбора конкретных вопросов.', false, false, true, NULL, 'group_system', 'Ключ', '{"key":{"_regex":"^[a-z0-9]+(?:-[a-z0-9]+)*$"}}'::json, 'Используйте латиницу, цифры и дефисы, например passport-warranty.');
