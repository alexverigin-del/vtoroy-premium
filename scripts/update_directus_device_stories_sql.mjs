...
WITH stories(device, story_title, story_body, story_facts) AS (
  VALUES
    (
      ...
      'Эта вещь пришла от постоянного клиента ISVOI: использовалась как основной рабочий телефон для встреч, поездок и деловой связи. Перед передачей данные удалены, устройство прошло диагностику, а небольшие следы на ребре уже учтены в грейде и цене.',
    ...
    (
      ...
      'iPhone перешёл в ISVOI после планового обновления на свежую модель. Им пользовались бережно: без ремонта, без следов влаги и без спорных сервисных вмешательств. Поэтому история вещи короткая и понятная: покупка, аккуратное использование, проверка, новая карточка в Store.',
